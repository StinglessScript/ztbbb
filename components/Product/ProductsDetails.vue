      ...
      <div class="row g-0 product">
        <div class="col-lg-4 col-md-12 col-sm-12">
          <div class="thumb-example">
            ...
            <!-- <div class="swiper gallery-thumbs d-flex" ref="a">
              <swiper-slide
                v-for="(imgUrl, i) in ztbeevn"
                :key="i"
                :class="{ 'swiper-slide-active': isActiveThum === i }"
              >
                <img
                  :src="imgUrl.imgUrl"
                  alt=""
                  ...
                  @mouseover="onThumbnailChange(i)"
              /></swiper-slide>
              <button style="position: absolute" @click="next">next</button>
            </div> -->
            <!-- <swiper
                ...
                <client-only>
                  <template v-for="(detail, idO) in details">
                    ...
                    >
                      ...
                      <span>Mô tả tại sao sp bắt buộc</span>
                    </div> -->
                      ...
                      <div class="norequire-item">
                        <div class="item-header">
                          ...
                          <div class="item-price">
                            <span
                              class="price-compare"
                              v-if="detail.compare_at_price > 0"
                            >
                              {{ convertPrice(detail.compare_at_price) }}
                              {{ detail.currency_code }}
                            </span>
          ...
          <div class="products-details">
            ...
            <div class="wrapper">
              <div class="price">
                ...
                <div class="products-price">
                  <span class="price-compare" v-if="product.compare_at_price">
                    {{ convertPrice(product.compare_at_price) }}
                    {{ product.currency_code }}
                  </span>
                  <span class="price-color">
...
export default {
  ...
  mounted() {
      ...
      for (let index = 0; index < item.length; index++) {
        ...
        if (this.id === element.id || item.length === 1) {
          this.$refs.items.filter((item, index) => {
            if (element.options.indexOf(`${item.innerText}`.trim()) !== -1) {
    ...
    this.default.price = this.product.price;
    this.default.slug = this.product.handle;
    this.default.id = this.product.id;
  },
  ...
  methods: {
        ...
        this.details.forEach((element, index) => {
          if (element.select_type_option == "OPTION_REQUIRE") {
            ...
            this.productOption[`${index}`] = {
              selectedSub: "",
              isSuccessSub: element.variants ? false : true,
              options_relationshipSub: [],
              options_relationshipSub1: {},
              name: {},
              checked: element.variants ? false : true,
            };
    ...
    handleShow() {
      this.details.forEach((element, i) => {
        if (element.product_type === "VARIABLE" && element.variants) {
          if (element.select_type_option !== "OPTION_REQUIRE") {
    ...
    handleVariants(name, value) {
      if (value === this.selectedOption[name]) {
        ...
        // history.pushState({}, null, this.default.slug);
        // this.getProductOptions(this.default.id);
      } else {
